...
export default function BorrowRequestIndex() {
    ...
    const [url, setUrl] = useState(`${baseUrl}/api/admin/borrow-requests?with=user,handler,borrowDetails.itemUnit,returnRequest`)
    ...
    useEffect(() => {
        const newUrl = `${baseUrl}/api/admin/borrow-requests?with=user,handler,borrowDetails.itemUnit,returnRequest&search=${searchTerm}&sortBy=${sortBy}&sortDir=${sortDir}`;
        setUrl(newUrl);
        setReload(true)
    ...
    const { isLoading, error, result } = useApi<PaginationResponse<BorrowRequest>>({
        url: url,
        headers: {
        ...
        <div className="container mx-auto py-4">
            <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage>Borrow Requests</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            ...
            {!error && !isLoading && result && <DataTable columns={BorrowRequestColumn} data={result.data} />}
